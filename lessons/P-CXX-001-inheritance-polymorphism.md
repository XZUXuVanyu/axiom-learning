...
### U4 learner field-placement revision

2026-09-07: Given path-identified input, the learner placed relative/absolute path, name, and extension in the common importer context and placed `error code` in `SystemError`. The learner assigned path/extension to logs and error code to retry logic. Review: a stored path normally already determines name and extension, so keeping all three duplicates information; choose one canonical document identifier first. The shared base also needs one stable human-readable description for base-level reporting (`what()`), though it need not be independently overridden in every derived type. Error codes are useful to logging and may inform retry policy, but an error code alone does not decide whether retry is correct; the failed operation and error category matter. The learner must now make one compact revised field list with reasons.
